...
module Verse
  module Distributed
    module Impl
      # In-memory implementation of Verse::Distributed::Hash (simple key-value store).
          ...
          @cleanup_thread = Thread.new do
            loop do
              ...
              begin
                ...
                Verse.logger&.warn "Error in DistributedHash cleanup thread: #{e.message}\n#{e.backtrace.join("\n")}"
                ...
                synchronize { @cond.wait(1) unless @stop } # Short wait on error, check stop again
              end
            end
          end
        end
